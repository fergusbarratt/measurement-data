...
data_loc = '../generate_x_data/processed_data/'
...
def decode_logits(record, n=None):
    return decode(record, n=n).real
...
if __name__ == "__main__":
    ...
    for path in paths:
        ...
        if float(p) == args.p and int(L)==args.L:
            ...
            arr = np.load(path)
            print(arr.shape)
            records.append(arr[:, :, :, :end(int(L))])
            samples.append(int(n_samples))
            labels.append(np.sum(arr[:, :, :, -1], axis=-1))
    ...
    if decode_type == 'argmax':
        ...
        print(result.shape, np.array(labels).shape, labels)
        path = f'logits/{np.sum(samples)},{args.L},{args.p}_label.npy'
        np.save(path, np.array(labels[0]))
