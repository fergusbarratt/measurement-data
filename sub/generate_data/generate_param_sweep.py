...
parser.add_argument('-N_batches', nargs='?', default=9, type=int) # this will make about 10000 samples
...
dup = 2*parser.dup
string = ""
ps = [0.025, 0.0375, 0.05, 0.0625, 0.075, 0.0875, 0.1, 0.125, 0.15, 0.2, 0.25, 0.3, 0.35, 0.45, 0.5] if parser.ps is None else parser.ps
Ls = [6, 8, 10, 12, 14, 16, 18, 20] if parser.Ls is None else parser.Ls
per_process = 90 # 9 batches per process - i.e. each process generates 1080 samples 
for L in Ls[::-1]:
    for p in ps:
