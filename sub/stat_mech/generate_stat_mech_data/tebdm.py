...
class TEBDm(qtn.tensor_1d_tebd.TEBD):
    ...
    def measure(self, odd=0):
        ...
        for i in range(self.L):
            if self.measurement_locations[2*int(self.t)+odd, i]:
                P0 = np.array([[1, 0], [0, 0]])
                P1 = np.array([[0, 0], [0, 1]])
                p_bloch = np.real(self._pt.copy(deep=True).gate_(P0, i, contract=True)@self.summer.H)

                outcome = 2*int(np.random.rand() < p_bloch)-1
                self.measurement_locations[2*int(self.t)+odd, i] = outcome
                ...
                if np.isclose(outcome, 1):  # 1 corresponds to rand() < ev(P0)
                    ...
                    if self.norm[-1]<=1e-16:
                        raise ValueError('sampled incompatible')
    ...
    def at_times(self, ts, measurement_locations=None, biases=None, *args, **kwargs):
        ...
        self.biases = biases if biases is not None else np.ones((self.L//2, 2*len(ts)+1)).T/2
    ...
    def sweep(self, direction, dt_frac=1, dt=1, queue=False):
        ...
        if direction == "right":
            ...
            for i in range(start_site_ind, final_site_ind, 2):
                ...
                bias = self.biases[2*int(self.t)+int(direction=='left'), int(direction=='left')+i//2]
                U = np.array(
                    [
                        [1, 0, 0, 0],
                        [0, 1-bias, bias, 0],
