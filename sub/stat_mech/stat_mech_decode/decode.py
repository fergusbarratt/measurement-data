...
data_loc = '../generate_stat_mech_data/data/'
    ...
    for L in Ls:
        for p in ps:
            ...
            def end(L):
                return 2*L
            for path in paths:
                L_, p_, *_ = str(path.stem).split(',')
                if float(p_) == p and int(L_)==L:
            ...
            K = None
            decode_type = 'logits'
            path = f'logits/{L},{p}.npy'
            cache_path = f'logits/data/{L},{p}.npy'
            ...
            if decode_type == 'argmax':
                ...
                others = list(Path('results/').glob(f'*{L},{p}.npy'))
                path = f'results/{np.sum(samples)},{L},{p}.npy'
                if not others:
