module Interleaved
    ...
    Sweep left and right and back again, appying born singlet/triplet measurements 
     or random unitaries 
    """
    function TEBDmeasuresweep!(A, p; dimerization=0, both=true, Un=SymMps.randomunitary, P₀=SymMps.P₀, P₁=SymMps.P₁, verbose = true, truncdim = 200, η = 1e-6)
        ...
        for k = N-1:-1:2
            ...
            if isodd(k) && both # update AAC if index is odd, else just shift the orth centre
                # if both is false, then just do the even sweep. 
                if rand() < pₒ 
                    @tensor p_born = AAC[a, b', c', d]*P₀[b', c', b, c]*conj(AAC[a, b, c, d])
                    if rand() < real(p_born)
                        @tensor AAC[a, b, c, d] := AAC[a, b', c', d]*P₀[b', c', b, c] # U_: V->V'
    ...
    function run(N, t_max, pₘ, γ=0, verbose=false, η=1e-6)
        ...
        allees = Array{Float64}(undef, N-1, t_max+2)
        for t = 1:t_max
            verbose && println("Time $t")
            allees[:, t] = real(SymMps.entanglemententropy!(A)) # get the entanglement entropy
            TEBDmeasuresweep!(A, pₘ, dimerization=γ, verbose=verbose, η = η, both=true);
            verbose && println()
        end
        Aₒ = deepcopy(A) # last applied is odd
        allees[:, t_max+1] = real(SymMps.entanglemententropy!(A)) # get the entanglement entropy
        TEBDmeasuresweep!(A, pₘ, dimerization=γ, verbose=verbose, η=η, both=false) # do a final even sweep, without applying on return 

        Aₑ = deepcopy(A) # last applied is even
        allees[:, t_max+2] = real(SymMps.entanglemententropy!(A))
        return (Aₒ, Aₑ), allees
    end

    function runmany(n_runs, N, t_max, pₘ, dirname="data", proc_id="", verbose=true, η = 1e-6; γ=0)
        # this is where the data goes
        out = Array{Float64}(undef, n_runs, N-1, t_max+2)
        ...
        Threads.@threads for n = 1:n_runs
            # do the run
            (Aₑ, Aₒ), out[n, :, :] = run(N, t_max, pₘ, γ, verbose, η)

            # save the run.
            id = SymMps.save(Aₑ, dir, "e")
            SymMps.save(Aₒ, dir, "o", id) # save the even and the odd final state. 
        end
