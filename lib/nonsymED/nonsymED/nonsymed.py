...
def apply_random_unitaries(psi, even, random_unitary=random_u1_unitary):
    ...
    for site in range(even, N-1, batch*2):
        ...
        if site + batch*2 > N:
            ...
            op = reduce(np.kron, [random_unitary() for _ in range(leftover_batch)]) if leftover_batch > 1 else random_unitary()
            applied_weight+=int(np.log2(op.shape[0]))
        else:
            op = reduce(np.kron, [random_unitary() for _ in range(batch)]) if batch > 1 else random_unitary()
            applied_weight+=int(np.log2(op.shape[0]))
